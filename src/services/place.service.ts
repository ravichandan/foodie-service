...
export class PlaceService {
    ...
    async getPlaces(params?: { placeName?: string, postcode?: string, pageSize?: number, pageNumber?: number }): Promise<IPlace[] | undefined> {
        log.info('Received request to getPlaces, params: ', params);

        const prams: { pageSize: number, pageNumber: number } = {...params, pageNumber: params?.pageNumber ?? 1, pageSize: params?.pageSize ?? 3}
        log.trace('Received request to getPlaces, prams: ', prams);

        try {
            let places;
            if (!!params?.placeName && !!params?.postcode) {
                places = await Place.find({placeName: {$regex: params?.placeName, $options: "i"}, "address.postcode": params.postcode}).populate('address').skip(prams.pageSize * (prams.pageNumber - 1)).limit(prams.pageSize).lean().exec();
            } else if (!!params?.placeName) {
                places = await Place.find({placeName: {$regex: params?.placeName, $options: "i"}}).populate('address').skip(prams.pageSize * (prams.pageNumber - 1)).limit(prams.pageSize).lean().exec();
            } else if (!!params?.postcode) {
                places = await Place.find({"address.postcode": params.postcode}).populate('address').skip(prams.pageSize * (prams.pageNumber - 1)).limit(prams.pageSize).lean().exec();
            } else {
                places = await Place.find({}).populate('address').skip(prams.pageSize * (prams.pageNumber - 1)).limit(prams.pageSize).lean().exec();
            }
