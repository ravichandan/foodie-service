...
export class MediaService {
	...
	async addMediaToR2(customerId: string, files: any) {
		...
		return await Promise.all((files as any).map(async (file: Express.Multer.File) => {

			// Write your buffer
			let bufferStream = Readable.from(file.buffer);
	...
	async removeMediaFromR2(files: any) {
		...
		return await Promise.all((files as any).map(async (file: any) => {
			// Write your buffer
...
// }
	//get a single media
	async getMedia(query: { id?: number; correlationId?: string }): Promise<IMedia | null> {
	...
	//update a media
	async updateMedia(id: number, data: any): Promise<IMedia | undefined> {
