...
export default [
...

	{
		...
		handler: (req: Request, res: Response) => {
			...
			if (!err.isEmpty()) {
				...
				const e = err.mapped();
				if (e.token)
					res.status(401).send({  token: {...e.token, value: undefined } });
				else
	...
	},
	{
		...
		handler: async (req: Request, res: Response) => {
			...
			if (!err.isEmpty()) {
				...
				// No errors, pass req and res on to your controller
				log.debug('in get /places/:placeId/items/:itemId/reviews route handler, processing request, req.params:', req.params);
				await reviewController.getReviews(req, res);
				log.debug('Returning the fetched reviews');
			}
		},
	...
	// Customers routes
	{
		...
		validators: [
			checkSchema(FieldConfigs.getOrPutCustomerByIdSchemaConfig),
		],
		handler: async (req: Request, res: Response) => {
			const err = validationResult(req);
			if (!err.isEmpty()) {
		...
		method: 'get',
		validators: [
			checkSchema(FieldConfigs.getCustomerByNameSchemaConfig),
		],
		handler: async (req: Request, res: Response) => {
			const err = validationResult(req);
			if (!err.isEmpty()) {
