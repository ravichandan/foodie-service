...
type Route = {
	path: string;
	method: string;
	validators: RunnableValidationChains<ValidationChain>[] | undefined;
...
[{ 'name': 'a', 'id': 1 }, { 'name': 'b', 'id': 2 }, { 'name': 'c', 'id': 3 }].reduce((a, v) => ({
	...a,
	[v.id]: v,
}), {});
...
export const placeToPlaceModel = (place: IPlace): PlaceModel => {
	...
	const model: PlaceModel = {
		...
		id: place._id,
		items: itemsToItemModels(place.items)?.reduce((accumulator, value) => ({
			...accumulator,
			[value.id]: value,
		}), ({} as any)),
...
export const reviewToReviewModel = (pi: IReview): ReviewModel => {
	...
	const model: ReviewModel = {
		...
		customerInfo: (typeof pi.customer === 'number') ? {id: pi.customer} : customerToCustomerModel(pi.customer as unknown as ICustomer),
...
export const calculatePoints = (action: 'react' | 'rate' | 'review' | 'comment' | 'image' | 'video' | string | undefined): number => {
