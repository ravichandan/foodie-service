...
class ReviewController {
	...
	getReviews = async (req: Request, res: Response) => {
		const { placeId, itemId, pageNumber = 1, pageSize = 10 }: {
			placeId: number;
			itemId?: number;
			pageNumber: number;
			pageSize: number
	...
	updateReview = async (req: Request, res: Response) => {
		...
		if(!!liked){
			...
			if (typeof index === 'number' && index > -1) { // only splice array when item is found
		...
		let review:IReview|null =   await reviewService.getReview({ id: data.id });
